 ...
 *
 *   DATA  A5 | id | seq | total | bytes...      chunk 0's bytes start with kind(1) len(2) 0(1)
...
object Transfer {
    ...
    fun chunks(id: Int, kind: Int, content: ByteArray): List<ByteArray>? {
        ...
        for (seq in 0 until total) {
            val head = if (seq == 0) byteArrayOf(kind.toByte(), (content.size shr 8).toByte(), content.size.toByte(), 0) else ByteArray(0)
    ...
    /** Kind and content from a complete set of chunk bytes (tag and header stripped). */
    fun assemble(parts: List<ByteArray>): Pair<Int, ByteArray>? {
        ...
        return if (body.size == len) kind to body else null
    }
